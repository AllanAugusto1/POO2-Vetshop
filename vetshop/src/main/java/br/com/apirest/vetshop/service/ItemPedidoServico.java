...
public class ItemPedidoServico implements IBaseServicoJPA <ItemPedido> {
    ...
    public ItemPedido Edit(ItemPedido obj) {
        Optional<ItemPedido> itemPedidoExistente = this.repositorio.findById(obj.getId());
        if(itemPedidoExistente.isPresent()){
            ItemPedido ite = itemPedidoExistente.get();
            ite.setProduto(obj.getProduto());
            this.repositorio.save(ite);
        }
        return null;
    }
