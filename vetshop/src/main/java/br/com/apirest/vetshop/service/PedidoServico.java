...
public class PedidoServico implements IBaseServicoJPA<Pedido>{
    ...
    public Pedido Edit(Pedido obj) {
        ...
        if(pedidoExistente.isPresent()){
            Pedido pe = pedidoExistente.get();
            pe.setNome(obj.getNome());
            // pe.setCliente(obj.getCliente());
            // pe.setFornecedor(obj.getFornecedor());
            // pe.setDataDeInclusao(obj.getDataDeInclusao());
            pe.setDataAlteracao(obj.getDataAlteracao());
            this.repositorio.save(pe);
        }
        return null;
    }
